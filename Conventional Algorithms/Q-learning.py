    ...
    for episode in range(MAX_EPISODES):
        ...
        while not is_terminal:
            ...
            if next_state == 'terminal':
                ...
                delta = reward + GAMMA * q_table.iloc[next_state, :].max() - q_table.loc[state, action]  # Q-Learning关键
...
def main():
    ...
    print("Q table\n{}\n".format(q_table))
    print('end')
    np.save('Q-learning_q_table.npy', q_table)
    np.save('Q-learning_step_counter_times.npy', step_counter_times)
    ...
    plt.title('Q-Learning Q (Reward) Tables', fontsize=14, loc="center", pad=20)

    # 绘制子图2，步数
    plt.subplot(1, 2, 2)
    plt.plot(step_counter_times, 'g-', label='Q-Learning')
    plt.xlabel('episode num')
    plt.ylabel("steps")
    plt.title('Q-Learning steps in each episode', fontsize=14, loc="center", pad=20)
    plt.legend()  # 显示图例
    plt.grid(True, alpha=0.3)  # 添加半透明网格
    plt.tight_layout()  # 自动优化间隔布局
    plt.savefig("Q-learning.pdf", format='pdf', bbox_inches='tight')
    plt.show()
